...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=False):
        ...
        src_logits = src_logits.permute(0, 2, 1).reshape(bs, hw, k)
    ...
    def forward(self, outputs, targets):
        ...
        indices, indices_aug = self.matcher(outputs_without_aux, targets)
        ...
        return losses, indices, indices_aug
...
class MinCostMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        batch_out_prob = outputs["pred_logits"].permute(0, 2, 1).reshape(bs, hw, k).sigmoid() # [batch_size, num_queries, num_classes]
        batch_out_bbox = outputs["pred_boxes"].permute(0, 2, 1).reshape(bs, hw, 4) # [batch_size, num_queries, 4]
        ...
        for i in range(bs):
            ...
            if tgt_ids.shape[0] == 0:
                ...
                indices_aug.append((torch.as_tensor([]).to(batch_out_prob), torch.as_tensor([]).to(batch_out_prob)))
            ...
            _, _src_ind = torch.min(C, dim=0)
            _tgt_ind = torch.arange(len(tgt_ids)).to(src_ind)
            indices_aug.append((torch.cat([src_ind, _src_ind]), torch.cat([tgt_ind, _tgt_ind])))
            
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices], \
    [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices_aug]
